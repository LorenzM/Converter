...
{
	// Datenstruktur bef�llen
	FillStructureWithPicInfo();

	// Farbraum der gespeicherten Daten �ndern
	ChangeColorModel();
}
...
void TJpegConverter::FillStructureWithPicInfo()
{
	...
	if (input != NULL)
	{
		...
		//... und danach die dazu geh�rige Ziffer.
		value = fgetc(input);
		magic_number += value;
		...
		if (value == "#")
		{
			// Wir ignorieren hier die Zeile mit der Raute ~ diese ist n�mlich unbrauchbare Information!
			while (value != "\n")
				value = fgetc(input);
		...
		while (total_pixel_count >= FPictureVec.size())
		{
			...
			if (value == " " || value == "\n")
			{
				//Hiermit legen wir fest welchen Farbwert wir als n�chstes auslesen.
			...
			else if (!feof(input) || durchlaufzaehler == 4)
			{
				...
				{
					// Wenn wir hier angekommen sind, haben wir alle 3 Farbwerte bef�llt und k�nnen diese in unsere Datenhaltung speichern.
					FPictureVec.push_back(new TRGBContainer(atoi(color_red.c_str()), atoi(color_green.c_str()), atoi(color_blue.c_str())));

					// Wir haben ein neues Objekt erzeugt, jetzt m�ssen wir unseren Z�hler auch zur�cksetzten!
					durchlaufzaehler = 1;

					// Zur�cksetzten nicht vergessen.
					color_red = "";
					color_green = "";
					color_blue = "";
